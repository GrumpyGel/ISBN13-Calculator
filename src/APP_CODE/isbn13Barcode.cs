...
**  Code taken from :                                                                                        **
**                                                                                                           **
**  https://stackoverflow.com/questions/13072676/how-do-i-create-a-dynamic-isbn-13-barcode-image-in-c-sharp  **
**                                                                                                           **
**  Code is as-is from the StackOverflow example, it has not been validated to be correct.                   **
**  Only changes are addition of "Using" references and "GM:" comments to fix syntax errors relating         **
**  array handling.                                                                                          **
**                                                                                                           **
**************************************************************************************************************/
...
public Bookland(string ISBN) {
  ...
  // GM: char[] isbn = ISBN.ToArray();

  char[] isbn = new char[ISBN.Length]; 
    ...
    using (Pen pen = new Pen(Color.White)) {
      using (Graphics g = Graphics.FromImage(bmp)) {
        ...
        for (int i = 0; i < binary.Length; i++) {
          ...
          // GM: if (delim.Contains(i)) {

          if (Array.Exists(delim, element => element == i)) {
